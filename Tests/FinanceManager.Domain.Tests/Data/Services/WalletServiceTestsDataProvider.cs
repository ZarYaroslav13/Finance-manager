using FinanceManager.Domain.Models;
using FinanceManager.Infrastructure.Models;

namespace FinanceManager.Domain.Tests.Data.Services;

public static class WalletServiceTestsDataProvider
{
    public static IEnumerable<object[]> GetAllWalletsOfAccountTestData { get; } = new List<object[]>
    {
        new object[]
        {
            new List<Wallet>
            {
                new Wallet { Id = Guid.Parse("1"), Balance = 100, UsertId = Guid.Parse("1"), Name = "Wallet1" },
                new Wallet { Id = Guid.Parse("2"), Balance = 200, UsertId = Guid.Parse("1"), Name = "Wallet2" }
            },
            1
        }
    };

    public static IEnumerable<object[]> AddWalletTestData { get; } = new List<object[]>
    {
        new object[]
        {
            new WalletModel()
            {
                Id = Guid.Parse("0"),
                AccountId = Guid.Parse("1"),
                Balance = 1000,
                Name = "Test"
            },
            new Wallet()
            {
                Id = Guid.Parse("0"),
                UsertId = Guid.Parse("1"),
                Balance = 1000,
                Name = "Test"
            }
        }
    };

    public static IEnumerable<object[]> UpdateWalletTestData { get; } = new List<object[]>
    {
        new object[]
        {
            new WalletModel()
            {
                Id = Guid.Parse("1"),
                AccountId = Guid.Parse("1"),
                Balance = 1000,
                Name = "Test"
            },
            new Wallet()
            {
                Id = Guid.Parse("1"),
                UsertId = Guid.Parse("1"),
                Balance = 1000,
                Name = "Test"
            }
        }
    };

    public static IEnumerable<object[]> IsAccountOwnerWalletAsyncInvalidAccountIdOrWalletIdThrowsArgumentOutOfRangeExceptionTestData { get; } = new List<object[]>
    {
        new object[] { -1, -1},
        new object[] { -1, 0},
        new object[] { 0, -1},
        new object[] { 1, 0},
        new object[] { 0, 1},
        new object[] { 0, 0},
    };
}
